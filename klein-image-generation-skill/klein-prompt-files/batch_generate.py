...
it's good" into a library of settings you can rely on.
  ...
  python batch_generate.py --plan plan.json \
      --workflow ~/ComfyUI/user/default/workflows/klein_t2i_api.json \
      --out ~/kids-art/run-2026-09-19

Requires: a ComfyUI workflow exported in **API format** (Workflow -> Export
(API)). Standard-format exports will not queue. Only the standard library is
used, so no pip install is needed.
...
def patch(wf, roles, prompt, params, seed, overrides):
...
def main():
    ...
    ap.add_argument("--out", default="./klein-out")
    ...
    roles = find_nodes(wf)
    overrides = {"positive": a.positive_node, "sampler": a.sampler_node, "latent": a.latent_node}
    ...
    for img in plan["images"]:
        ...
        for seed in seeds:
            body = patch(wf, roles, img["prompt"], img["params"], seed, overrides)
            if a.dry_run:
                print("DRY RUN %s seed=%d %dx%d steps=%s cfg=%s" % (
                    img["id"], seed, img["params"]["width"], img["params"]["height"],
                    img["params"]["steps"], img["params"]["cfg"]))
                continue
            resp = http_json("%s/prompt" % base,
                             {"prompt": body, "client_id": "klein-kids-art-%d" % random.randrange(1 << 30)})
            ...
            with open(manifest_path, "a", encoding="utf-8") as fh:
                fh.write(json.dumps({
                    ...
                    "params": img["params"], "pack": plan["pack"], "age": plan["age"],
