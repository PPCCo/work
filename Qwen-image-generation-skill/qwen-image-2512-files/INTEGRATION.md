...
`book-cover-*`. The interior-art skill is bundled at
`.claude/skills/qwen-kids-art/` in this package, and `cover_prompt.py`
auto-discovers it at exactly that path — no environment variable needed for a
standard install. Set `QWEN_SKILL_PATH` only if you keep the skill somewhere
else:

```bash
# in .claude/settings.local.json or the shell that runs Claude Code, only if
# qwen-kids-art is NOT at .claude/skills/qwen-kids-art
export QWEN_SKILL_PATH=/path/to/qwen-kids-art
```

Python needs Pillow (`pip install Pillow`); `pypdf` is optional and only used
to measure PDF wraps. Fonts must be installed locally — the composer refuses
to invent type.

**Before generating anything, confirm your ComfyUI install can actually run
Qwen-Image-2512.** It's a 20B-parameter model — five times the size of the
FLUX.2 [klein] 4B model this pack used to target — and the Q8_0 GGUF alone is
roughly 20 GB on disk, with a comfortable VRAM target of 24 GB or more. See
`.claude/skills/qwen-kids-art/references/comfyui-runtime.md` #1–3 for exact
file names, loader nodes (`ComfyUI-GGUF` custom node required) and what to do
if VRAM is tight. If your box was sized for the old 4B setup, check this
before building a pipeline around it, not after.
...
.claude/
├── agents/
│   ├── cover-art-director.md      decides the look, writes audited Qwen-Image prompts
...
│   ├── book-cover-art/            /book-cover-art     Qwen-Image artwork
...
└── scripts/cover/
    ...
    ├── cover_prompt.py      Qwen-Image prompts (positive + negative) with reserved type zones + upscale plan
...
python .claude/scripts/ph_cli.py artifact register <id> \
  ...
  --actor cover-typographer --provenance-json '{"model":"qwen-image-2512-q8_0","seeds":[...]}'
...
**Qwen-Image-2512 cannot reach print resolution alone.** Its native
resolutions top out around 1.3–1.8 MP; a 300 DPI print front is 3–9 MP.
`cover_prompt.py` emits the required upscale factor and expects a ComfyUI
upscale model in the chain. Flat vector and graphic packs upscale almost
perfectly, which is part of why those are the defaults for activity and
puzzle lines. Painted packs still lose the most detail relative to a full
native render. The composer will upscale as a fallback and warn loudly —
treat that warning as a build defect, not a notice.

**The negative prompt only works on the `full` runtime tier.** Unlike the old
Klein setup, Qwen-Image-2512 has a real negative-prompt channel — but it's
inert on the fast `lightning` tier (CFG forced to 1.0). Cover art defaults to
`full` in every cover skill and agent in this pack, since a cover is almost
always a keeper rather than a draft; if you override to `lightning` for quick
composition exploration, know that any `--negative-extra` you pass is
silently ignored until you re-render on `full`.
