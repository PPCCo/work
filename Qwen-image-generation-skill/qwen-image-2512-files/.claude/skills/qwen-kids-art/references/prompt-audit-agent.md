...
```bash
python scripts/audit_prompt.py --pack storybook_gouache --age 5-7 \
    --prompt "<the prompt>" --negative "<the negative prompt, if any>" \
    --variant full
```

It returns a score out of 100 with blockers, warnings and notes. Pass
`--negative` and `--variant` even when drafting by hand - the audit checks
that a negative prompt is only present when it can actually do something
(§8 of `qwen-prompt-standards.md`: it's inert on `lightning`). Blockers mean do not
...
**D. Is anything that should be excluded sitting in the positive prompt
instead of the negative field?** A phrase like "not too dark" or "no extra
fingers" in the positive prompt is a Klein-era habit that no longer applies -
Qwen-Image has a real negative channel now. Move it.

**E. Is the style signature byte-identical to the pack?**
...
`references/qwen-prompt-standards.md`, change the prompt or the runtime, and note
