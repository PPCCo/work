...
| Layer | Owner | Tool |
|---|---|---|
| Artwork | Qwen-Image-2512 (Q8_0) via ComfyUI | `cover_prompt.py` → the generation MCP |
...
Any prompt that asks the image model for a title, a subtitle, an author name,
a series name, a price or a barcode is a defect, and `cover_prompt.py` blocks
it. This holds even though Qwen-Image-2512 renders in-scene text considerably
better than the smaller model this pack used to run on: the reason for the
split is exact wording matching the metadata record, editability when a
subtitle changes, and font licensing — none of which is about whether the
model *can* render legible text.
